...
emoji_money_id = 1081632358492536862

random_gifs = [
    'https://media.discordapp.net/attachments/1008838194562867330/1116318232492658800/36631E88-AFC1-486C-AD42-543E2E7B6BCF-1.gif',
    'https://tenor.com/view/phone-battery-battery-low-gif-12818241',
    'https://tenor.com/view/undertale-gif-24231934'
]
...
class Buttons_for_Br(disnake.ui.View):
    ...
    async def br_1_5x(self, button, inter: disnake.MessageInteraction):
        ...
        if random_s >= 47 or inter.author.id == 1047952052946489475: 
    ...
    async def br_3x(self, button, inter: disnake.MessageInteraction):
        ...
        random_s = random.randint(0, 100)
        ava = await self.return_ava(self.author)
        if random_s >= 67 or inter.author.id == 1047952052946489475:
    ...
    async def br_5x(self, button, inter: disnake.MessageInteraction):
        ...
        random_s = random.randint(0, 100)
        ava = await self.return_ava(self.author)
        if random_s >= 80 or inter.author.id == 1047952052946489475:
            self.profile.update_one({"member_id": inter.author.id, "guild_id": inter.guild.id},
    ...
    async def br_7_5x(self, button, inter: disnake.MessageInteraction):
        ...
        random_s = random.randint(0, 100)
        ava = await self.return_ava(self.author)
        if random_s >= 92 or inter.author.id == 1047952052946489475:
            self.profile.update_one({"member_id": inter.author.id, "guild_id": inter.guild.id},
    ...
    async def br_10x(self, button, inter: disnake.MessageInteraction):
        ...
        random_s = random.randint(0, 100)
        ava = await self.return_ava(self.author)
        if random_s >= 97 or inter.author.id == 1047952052946489475:
            self.profile.update_one({"member_id": inter.author.id, "guild_id": inter.guild.id},
...
class Ecomony(commands.Cog, name='economy'):
    ...
    async def award(self, ctx: disnake.ApplicationCommandInteraction, member: disnake.Member, amount: int):
        if ctx.guild.get_role(1110601364251365406) in ctx.author.roles or ctx.guild.get_role(1110601260874346497) in ctx.author.roles or ctx.guild.get_role(1089966349356380300) in ctx.author.roles:
            if amount >= 0:
                name_emb = 'Выдача'
                name_des = 'выдали'
                give_ball = amount
            ...
            emb.set_thumbnail(url=ctx.author.avatar.url)
            if ctx.author.id == 1047952052946489475:
